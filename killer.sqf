...
if (_killedbyvehicle) then
	{
	(format ["[""KilledCiv"", %1] spawn Isse_AddCrimeLogEntry; if(%1 in civarray)then{if(!(""Hit and Run"" in %1_reason))then{%1_reason = %1_reason + [""Hit and Run""]}; kopfcash_%1 = kopfcash_%1 + 7500};", player]) call broadcast;
	}
	else
	{
	(format ["[""KilledCiv"", %1] spawn Isse_AddCrimeLogEntry; if(%1 in civarray)then{if(!(""murder"" in %1_reason))then{%1_reason = %1_reason + [""murder""]}; kopfcash_%1 = kopfcash_%1 + 10000};", player]) call broadcast;
	};
...
if (_killedbyvehicle) then
	{
	(format ["[""KilledCop"", %1, ""%2""] spawn Isse_AddCrimeLogEntry; if(%1 in civarray)then{if(!(""Hit and run on a cop"" in %1_reason))then{%1_reason = %1_reason + [""Hit and run on a cop""]}; kopfcash_%1 = kopfcash_%1 + 10000};", player,  _killer]) call broadcast;
	}
	else
	{
	(format ["[""KilledCop"", %1, ""%2""] spawn Isse_AddCrimeLogEntry; if(%1 in civarray)then{if(!(""cop murder"" in %1_reason))then{%1_reason = %1_reason + [""cop murder""]}; kopfcash_%1 = kopfcash_%1 + 12500};", player,  _killer]) call broadcast;
	};
