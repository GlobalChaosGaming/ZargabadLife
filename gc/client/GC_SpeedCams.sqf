...
while {alive _veh and _veh != player and player == driver _veh} do {
	_speed = speed _veh;
	{
		...
		if (_veh distance _obj < 30 and _speed >= 77 and !(_veh iskindof "Air")) then {
			...
			_ticket = _penalty * 75;
			if (iscop) then {_ticket = _ticket * 2;};
			...
			if (!(_license call INV_HasLicense) and !_alreadytagged) then {
				format['server globalchat "%1 was photographed on %2 speeding without a license!";',name player, (_x select 1)] call broadcast;
				format [" [""Speed"", %1] spawn Isse_AddCrimeLogEntry; if(!(""excessive speeding"" in %1_reason))then{%1_reason = %1_reason + [""excessive speeding""]; kopfcash_%1 = kopfcash_%1 + 1000;}; %1_wanted = 1;", player] call broadcast;
				_alreadytagged = true;
			};
			...
			if(demerits == 0) then {
				...
				format [" [""Speed"", %1] spawn Isse_AddCrimeLogEntry; if(!(""excessive speeding"" in %1_reason))then{%1_reason = %1_reason + [""excessive speeding""]; kopfcash_%1 = kopfcash_%1 + 500;}; %1_wanted = 1;", player] call broadcast;
			};
		};
