...
_victimside = side vehicle player;
...
if (vehicle player == _killer) then {
	...
	if (!isNull _vehx) then {
		if (count crew _vehx > 0 and speed _vehx != 0) then {
...
_killerside1 = side _killer;

_playerSide = "";
if (_victimside == CIVILIAN) then {_playerSide = "civ"};
...
_killerSide = "";
if (_killerside1 == CIVILIAN) then {_killerSide = "civ"};
if (_killerside1 == WEST or _killer in coparray) then {_killerSide = "cop"};
