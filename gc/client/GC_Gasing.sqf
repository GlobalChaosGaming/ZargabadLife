...
while { (time < (_timenow + 29) && (!(isnull _bullet))) } do {
	_pos = getpos _bullet;
	{
		...
		if (isPlayer _x and alive _x) then { 
			_x setVariable ["gassed",true, true];
			_array set [count _array,_x];			
		};
	} foreach nearestObjects [_pos, ["Man"], 25];  //not checking landvehicles  only effect bikes and atvs....
	sleep 1;
};
sleep 4;
{
	_x setVariable ["gassed",false, true];
	_array = _array - [_x];	
} foreach _array;