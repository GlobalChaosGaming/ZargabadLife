...
if (_option == "arrest") then
  {
    ...
    if (_civkopfcash != 0) then
      {
        //Innocent - revisit - this is where we would record a sucessful arrest of a wanted criminal
        (format['
	...
	hintsilent parseText format["<t shadow='1' shadowColor='#cc0000' underline='1' size='1.3' font='Zeppelin33' align='center' valign='top'>Remaining sentence</t><br/><t align='center' valign='bottom' font='LucidaConsoleB'>%1</t><br/><t shadow='1' shadowColor='#cc0000' underline='1' size='1.3' font='Zeppelin33' align='center' valign='top'>Remaining bail</t><br/><t align='center' valign='bottom' font='LucidaConsoleB'>$%2</t>", _tur, round(_bail)];
...
if (_option == "stealmoney") then
  {
    ...
    (format ['if (rolestring == "%1") then {["cash", %2] call INV_AddInventoreItem;["SKILL_ROBBERY",0.025] call GC_addSkill;};hint "%1 stole %2 from %3"; ["Stole", %1, %3, %2] spawn Isse_AddCrimeLogEntry;',_aktionsStarter, _amounttosteal, player]) call broadcast;
