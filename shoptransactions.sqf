...
if (_itemart == "item") then
	...
	if (!(_license1 call INV_HasLicense) and isciv and _license) exitWith {player groupChat format[localize "STRS_inv_sellitems_nolicense", (_license1 call INV_GetLicenseName)]; _exitvar = 1};
	if (!(_license2 call INV_HasLicense) and iscop and _license) exitWith {player groupChat format[localize "STRS_inv_sellitems_nolicensecop", (_license2 call INV_GetLicenseName)]; _exitvar = 1};
...
if (((INV_ItemShops select INV_ActiveShopNumber) select 0) == OilSell1) then{
	...
	sleep 0.1;
	if(TankenCost < minPetrolPrice) then {TankenCost = minPetrolPrice};
