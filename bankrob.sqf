...
if (_option == "ausrauben") then {
	...
	for [{_i=0},{_i<4},{_i=_i+1}] do {
		if (alive player && player distance _safe >= 7.5 && !(player distance _safe > 10)) then {player groupchat "You are too far away from the safe you need to stay within 10m!";};
		if (alive player && player distance _safe > 10) then{
			...
			sleep (_sleep / 4);
			if (_i==3 && alive player)then{player groupchat "The safe has been emptied!";};
		};
	};
	...
	if (alive player && player distance _safe < 20) then {
		...
		_addskill = ((local_cash / 150) * 0.0015);
		["SKILL_ROBBERY",_addskill] call GC_addSkill;
		local_cash = floor(local_cash + (2500 * SKILL_ROBBERY));
