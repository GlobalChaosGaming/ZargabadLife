...
if (_art == "spielerliste") then
{
	...
	for [{_i=0}, {_i < (count INV_ItemFabriken)}, {_i=_i+1}] do
	{
		if (((INV_ItemFabriken select _i) select 1) in INV_Fabrikowner) then
		{
			...
			_queue = call compile format['%1',((INV_ItemFabriken select _i) select 8)];
			lbAdd [1, format['%1 : %2 workers', ((INV_ItemFabriken select _i) select 2), _workers]];
			{lbAdd [1, format['	->%1',_x]];} foreach _queue;
		};
	};
	lbAdd [1, _trennlinie];
	...
	lbAdd [1, _trennlinie];
 	...
 	lbAdd [1, "S K I L L S:"];
 	lbAdd [1, format ["Fishing - %1",((SKILL_FISHING) call ISSE_str_StrToFloat)]];
	lbAdd [1, format ["Hunting - %1",((SKILL_HUNTING) call ISSE_str_StrToFloat)]];
	lbAdd [1, format ["Gathering - %1",((SKILL_GATHERING) call ISSE_str_StrToFloat)]];
	lbAdd [1, format ["Mining - %1",((SKILL_MINING) call ISSE_str_StrToFloat)]];
 	lbAdd [1, format ["Manufacturing - %1",((SKILL_MANUFACTURING) call ISSE_str_StrToFloat)]];
	lbAdd [1, format ["Lockpicking - %1",((SKILL_LOCKPICKING) call ISSE_str_StrToFloat)]];
	lbAdd [1, format ["Robbery - %1",((SKILL_ROBBERY) call ISSE_str_StrToFloat)]];
	lbAdd [1, format ["Prostitution - %1",((SKILL_PROSTITUTION) call ISSE_str_StrToFloat)]];
	lbAdd [1, format ["Exporting - %1",((SKILL_EXPORT) call ISSE_str_StrToFloat)]];
	...
	_demand = (((TankenCost - minPetrolPrice)/200)*100);
