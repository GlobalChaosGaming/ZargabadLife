...
if (_art == "create") then {
  ...
  if ((_text call ISSE_str_Length) > 30 or (_text call ISSE_str_Length) < 3) exitWith {player groupChat "Name is either too long or too short!";};
  ...
  player groupchat format["You've created a new gang called %1!", _text];
...
if(_art == "join") then {
  ...
  if (count _members > 4) exitwith{player groupchat "The gang is full, please try again later."};
...
if(_art == "kick") then {
  if ((_this select 1) == "notingame") exitwith {player groupchat "You cannot kick members unless they are in-game!"};
