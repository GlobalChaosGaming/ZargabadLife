...
if (_art == "use") then {
  if (!isCop) then {
    ...
    if (!(call GC_PlayerInCrimeFree)) then {
        ...
        if (_closeVcl in INV_VehicleArray) then {
      	...
      	publicVariable format["SFXPvEh_%1", rolenumber];
            ...
            if (([player, (civarray + coparray), 40] call INV_Seen or _incarpark) and !iscop) then {
              format [" [""StoleVcl"", %1, %3] spawn Isse_AddCrimeLogEntry; if(!(""vehicle theft"" in %1_reason))then{%1_reason = %1_reason + [""vehicle theft""]}; %1_wanted = 1; kopfcash_%1 = kopfcash_%1 + 2000; hint format[localize ""STRS_inventar_lockpick_gesehen"",%1, %3]; ", player, longrolenumber, _closeVcl] call broadcast;
            };
      ...
      publicVariable format["SFXPvEh_%1", rolenumber];
          ...
          ["SKILL_LOCKPICKING",_addskill] call GC_addSkill;
            ...
            if (([player, (civarray + coparray), 40] call INV_Seen or _incarpark) and !iscop) then {
              format [" [""AttemptedStoleVcl"", %1, %3] spawn Isse_AddCrimeLogEntry; if(!(""attempted vehicle theft"" in %1_reason))then{%1_reason = %1_reason + [""attempted vehicle theft""]}; %1_wanted = 1; kopfcash_%1 = kopfcash_%1 + 1000; hint ""%1 was seen attempting to lockpick a vehicle (Registration plate: %3)"";", player, longrolenumber, _closeVcl] call broadcast;
            };
    ...
    player groupChat "Cops cannot use lockpicks";
  };
};
